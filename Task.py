...
st.title(":blue[Youtube Data Harvesting and Warhousing]")

# Connecting with Youtube API
#Api_key = 'AIzaSyB-6MZUNT7w2MgW7Aj6YfB3duBAXInRGQE'
Api_key= 'AIzaSyDxzqxWFrzTN8mjpeVFi4JmYutjCRRFJ3Y'
...
with tab2:
    if st.button("Upload Data to MongoDB Atlas"):
        ...
        with st.spinner('Uploading....'):
            ...
            client = MongoClient(f"mongodb+srv://ms_mgr_007:muthu007@cluster0.dpdtdjk.mongodb.net/?retryWrites=true&w=majority")
            ...
            st.success("Data Migrate to MongoDB Atlas is Successfull", icon="✅")    
    ...
    if user_inp:
        ...
        # VALUES INSERTION INTO MYSQL     
                ...
                for video_item in mycol3.find({"channel_name": user_inp}, {'_id': 0}):
                    ...
                    mycursor.execute("INSERT INTO video_data VALUES ( %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", values)
                    mydb.commit()
...
with tab4:
    ...
    details = st.selectbox('Select any questions given below :', ['Click the question that you would like to query', q1, q2, q3, q4, q5, q6, q7, q8, q9, q10])
    ...
    if clicked:
        ...
        if details == q1:
            ...
            mycursor.execute(query)
            results = mycursor.fetchall()
            df = pd.DataFrame(results, columns=['Channel_name', 'Top_10_viewd_videos',"View_count"],index=np.arange(1, len(results) + 1))
            st.write(df)   
            ...
            mycursor.execute(query)
            results = mycursor.fetchall()
            df = pd.DataFrame(results, columns=['Channel_name', 'Video_name', 'Like_count'],index=np.arange(1, len(results) + 1))
            st.write(df)   
            ...
            mycursor.execute(query)
            results = mycursor.fetchall()
            df = pd.DataFrame(results, columns=['Channel_name', 'total_Channel_views'],index=np.arange(1, len(results) + 1))
            st.write(df) 
            ...
            mycursor.execute(query)
            results = mycursor.fetchall()
            df = pd.DataFrame(results, columns=['Channel_name', 'Average_duration'],index=np.arange(1, len(results) + 1))   
            st.write(df)
            ...
            mycursor.execute(query)
            results = mycursor.fetchall()
            df = pd.DataFrame(results, columns=['Video_name', 'Channel_name', 'Comment_count'],index=np.arange(1, len(results) + 1))
            st.write(df)           
