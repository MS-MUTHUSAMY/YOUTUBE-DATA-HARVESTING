...
pg_connection = mysql.connector.connect(
                                host="localhost",
                                database="task_db",
                                user="root",
                                password=""
...
if selected == "Migrate":
    ...
    def migrate_data():
        ...
        for video_item in collection2.find({"Channel_name": user_inp}, {'_id': 0}):
            ...
            pg_cursor.execute("INSERT INTO videos VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", values)
...
if selected == "View":
    ...
    if questions == '-1. What are the names of all the videos and their corresponding channels?':
        ...
        df = pd.DataFrame(query_result, columns=["Video name","Channel name"], index=np.arange(1, len(query_result) + 1))
        st.write(df)
        ...
        df = pd.DataFrame(query_result,columns=["Channel has more no of videos","Video counts"] , index=np.arange(1, len(query_result) + 1))
        st.write(df)
        ...
        #column_names = [desc.name for desc in pg_cursor.description] columns=["Top 10 most viewed videos","channel name"] ,

        df = pd.DataFrame(query_result,columns=["Top 10 most viewed videos","views count","channel name"] , index=np.arange(1, len(query_result) + 1))
        st.write(df)
        ...
        df = pd.DataFrame(query_result,columns=["Comment counts on video","Video name"] , index=np.arange(1, len(query_result) + 1))
        st.write(df)
        ...
        df = pd.DataFrame(query_result,columns=["Video name","Likes count","Channel name"] , index=np.arange(1, len(query_result) + 1))
        st.write(df)
        ...
        df = pd.DataFrame(query_result, columns=["Likes count","Video name"], index=np.arange(1, len(query_result) + 1))
        st.write(df)
        ...
        df = pd.DataFrame(query_result, columns=["Total no of views of the channel","Channel name"], index=np.arange(1, len(query_result) + 1))
        st.write(df)
        ...
        df = pd.DataFrame(query_result, columns=["Channel name"] ,index=np.arange(1, len(query_result) + 1))
        st.write(df)
        ...
        df = pd.DataFrame(query_result,columns=["Channel name","Average duration of all videos"]  ,index=np.arange(1, len(query_result) + 1))
        st.write(df)
        ...
        df = pd.DataFrame(query_result, columns=["Video name","Comment count","Channel name"], index=np.arange(1, len(query_result) + 1))
        st.write(df)
